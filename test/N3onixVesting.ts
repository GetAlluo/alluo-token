...
import { N3onixVesting } from '../typechain/N3onixVesting'
import { N3onixVesting__factory } from '../typechain/factories/N3onixVesting__factory'
...
describe('Contract: N3onixVesting', () => {
    let accounts: SignerWithAddress[];
    let investorsVesting: N3onixVesting;
    let token: AlluoToken;

    const cliffMonths = 6;
    const vestingMonthsCount = 36;
    const month = 2628000;
    ...
    beforeEach(async function () {
        ...
        let InvestorsVesting = await ethers.getContractFactory("N3onixVesting") as N3onixVesting__factory;

        token = await AcceptedToken.deploy(deployer) as AlluoToken;
        investorsVesting = await InvestorsVesting.deploy(token.address) as N3onixVesting;
