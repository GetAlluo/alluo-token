...
import { PartnerVesting } from '../typechain/PartnerVesting'
...
describe('Contract: PartnerVesting', () => {
    ...
    beforeEach(async function () {
        ...
        let InvestorsVesting = await ethers.getContractFactory("PartnerVesting") as PartnerVesting__factory;

        token = await AcceptedToken.deploy(deployer) as AlluoToken;
        investorsVesting = await InvestorsVesting.deploy(token.address) as PartnerVesting;
    ...
    it("Should add private user (user re-add check)", async function () {
        ...
        await investorsVesting.addPartner(users, amounts);
        ...
        await investorsVesting.addPartner(users, newAmounts);
    ...
    it("Should not add private user (not owner)", async function () {
        ...
        await expect(investorsVesting.connect(notAdmin).addPartner([], [])).to.be.revertedWith("Ownable: caller is not the owner");
    });
    ...
    it("Should not add private user (no tokens on contract)", async function () {
        ...
        await expect(investorsVesting.addPartner([account], [amount])).to.be.revertedWith("Vesting: total tokens to pay exceed balance");
    });
    ...
    it("Should not add private user (some amount is zero)", async function () {
        ...
        await expect(investorsVesting.addPartner(users, amounts)).to.be.revertedWith("Vesting: some amount is zero");
    });
